...
class LLMEngine:
    ...
    def step(self):
        ...
        """
        raise NotImplementedError("TODO: 看 learning-guide 对应章节，把这个函数填出来")  # LLMEngine.step

    # ---- 两种用法 ----

    def generate(self, prompts_token_ids, sampling_params=None):
        """同步生成：进一批、跑到完、按提交顺序返回 list[RequestOutput]。"""
        raise NotImplementedError("TODO: 看 learning-guide 对应章节，把这个函数填出来")  # LLMEngine.generate

    def stream(self, prompt_token_ids, sampling_params=None):
        """流式生成（单请求）：每走一步，吐出新生成的文本增量。"""
        raise NotImplementedError("TODO: 看 learning-guide 对应章节，把这个函数填出来")  # LLMEngine.stream
