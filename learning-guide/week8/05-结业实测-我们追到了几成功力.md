...
- **同一台机器**：本机 Mac（Apple Silicon，MPS 后端）；
...
**我们的 minivllm**（reference 实现）、**真实 vLLM**（vllm-metal 版）。
...
真实 vLLM 要用它自己的 venv，而且有三个"必须"（Week 1 见过，这里复习）：
...
- **必须从 .py 脚本文件运行**：vLLM 会 fork 子进程，不能用
  `python - <<EOF` 或 `python -c`；
...
> ⚠️ **易踩坑：** 跑实测最常见的两个乌龙——
> ① 用项目的 `.venv` 去跑 `bench_vllm.py`（真实 vLLM 只在它自己的
> vllm-metal venv 里，混用一定报 import 错误）；
