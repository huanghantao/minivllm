...
问得好。我们在这台 Mac 上做了个实测（Apple M3 Max、96 GB 内存，配置详见
第 1 章 1.2 节）：同一个 Qwen3-0.6B 模型、同一台机器，
...
> 📌 对标 vLLM：真实 vLLM 的"后厨管理"核心在源码包的
> `vllm/v1/core/sched/scheduler.py`（调度）和 `vllm/v1/engine/llm_engine.py`（引擎门面）。
> 第 1 章会把 vLLM 用 pip 装好——源码就在你的 `.venv` 里，随时可以打开。
