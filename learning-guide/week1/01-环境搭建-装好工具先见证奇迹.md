...
本课程在 macOS（Apple Silicon）上开发验证，**环境统一用 Python 3.12**（开发机就是 3.12；
1.5 节要装的真实 vLLM，其 Mac 预编译 wheel 和 vllm-metal 插件也都是 cp312 的）。

> 📌 **本书开发机的配置**——书里所有"本机实测"数字（比如 39.3 vs 145.8 token/s），
> 都出自这台机器：
>
> | 项目 | 配置 |
> |---|---|
> | 机型 | MacBook Pro 16 英寸（2023 款，型号 Mac15,11） |
> | 芯片 | Apple M3 Max：14 核 CPU（10 性能核 + 4 能效核）、30 核 GPU（Metal 3） |
> | 内存 | 96 GB 统一内存 |
> | 系统 | macOS 14.1（Sonoma） |
> | Python | 3.12（项目 `.venv`；实测 torch 2.13.0、真实 vLLM 0.27.1） |
>
> 你**不需要同款机器**：Apple Silicon 的 Mac（M1 及更新的芯片）都能按本书流程装环境、
> 跑完全部课程，内存也远用不到 96 GB（Qwen3-0.6B 权重才约 1.2 GB）。只是机器不同，
> 实测数字会略有出入，书里的结论不变。
...
看到 `Python 3.12.x` 就合格；如果不是 3.12，请先把 Python 3.12 装好再继续。
...
两个包都从 GitHub release 装——PyPI 上没有 Mac 版
...
运行（必须写成带 main 守卫的 .py 脚本文件，且必须加三个环境变量）：
...
| `VLLM_HOST_IP=127.0.0.1` | 本机开着代理工具（Clash Verge）的 **TUN 模式**时，系统默认路由被虚拟网卡 `utun4` 接管，vLLM 探测自己 IP 时会拿到 `198.18.0.1` 这个 fake-IP。内部的 TCPStore 建在它上面，连接一发包就被代理关掉，EngineCore 于是无限重试 `Failed to recv, got 0 bytes`。固定成回环地址即可。TUN 关着时不加也能跑，加了没有副作用。 |

> ⚠️ 易踩坑：最容易踩的四个坑——① 手快打成 `VLLM_METAL_USE_PAGED_ATTENTION=1`
> （方向反了，`0` 才是"退回兼容实现"）；② 图省事用 `python -c` 跑；
> ③ 脚本顶层直接写 `llm = LLM(...)` 而没套 `if __name__ == "__main__":`——
> 子进程会把顶层代码再跑一遍，报一大串 multiprocessing 的错；
> ④ 开着代理 TUN 模式却漏了 `VLLM_HOST_IP`——满屏 `Failed to recv, got 0 bytes`
> 看着像网络崩了，其实只是 vLLM 把自己的 IP 认错了，加回环地址就好。
> 四个坑都在上面表格里，遇到报错先回来对一遍。
...
命令跑不通时，对照 1.5 节的"四个机关"表格逐条排查，绝大多数问题是
`VLLM_METAL_USE_PAGED_ATTENTION` 写成了 `1`、用了 `python -c`、
忘了 `if __name__ == "__main__":` 守卫，或开着代理 TUN 却漏了 `VLLM_HOST_IP`。
