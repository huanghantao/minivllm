...
```
~/codeDir/pythonCode/vllm-metal/.venv-vllm-metal/bin/python
```
...
运行（必须用这个 venv，且必须加两个环境变量）：
    HF_HUB_OFFLINE=1 VLLM_METAL_USE_PAGED_ATTENTION=0 \
      ~/codeDir/pythonCode/vllm-metal/.venv-vllm-metal/bin/python hello_vllm.py
"""
...
| `VLLM_METAL_USE_PAGED_ATTENTION=0` | 本机是 macOS 14.1，vllm-metal 的预编译 Metal kernel 需要 macOS 15+。设为 `0` 退回 MLX 自带的注意力实现，否则会报错。**注意是 `=0`（关掉），不是 `=1`。** |
...
> 源码在 `~/codeDir/pythonCode/vllm` 仓库的 `vllm/entrypoints/` 一带。
