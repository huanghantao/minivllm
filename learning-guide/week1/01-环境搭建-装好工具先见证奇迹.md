...
本课程在 macOS（Apple Silicon）上开发验证，Python 用 3.10 以上即可（开发机是 3.12）。
先检查：
...
看到 `Python 3.10.x` 或更高就合格。太低（比如 3.8）请先装新一点的 Python。
...
真实 vLLM 装在另一个独立的虚拟环境里（它和本项目的依赖版本要求不同，分开装互不干扰）：
...
> 📌 **本书的路径约定（先读这一段）**
> 本书里凡是出现 `$VLLM_SRC` 和 `$VLLM_PY` 的，都是**占位符**，指**你自己机器上**的两个位置，每个同学的值都不一样：
> - `$VLLM_SRC` = 你 clone 下来的真实 vLLM 源码仓库的**根目录**（想跟着读源码、跑对标实验的同学需要先准备一份）；
> - `$VLLM_PY` = 装有真实 vLLM 的那个虚拟环境的 **python 解释器**。
>
> 想照抄命令直接跑的同学，先在自己的终端里 export 两个变量（路径换成你自己的）：
> ```bash
> export VLLM_SRC=$HOME/projects/vllm                      # 你的 vLLM 源码仓库根目录
> export VLLM_PY=$HOME/projects/vllm/.venv/bin/python      # 你的 vLLM 虚拟环境 python
> ```
> 还没有 vLLM 源码的同学，`git clone https://github.com/vllm-project/vllm` 一份即可。
> 只想读不跑的同学，把 `$VLLM_SRC` 当成"你自己的 vLLM 仓库路径"看就行。
...
| 机关 | 为什么必须 |
|---|---|
| 写成 `.py` 脚本文件运行 | vLLM 内部会 fork 子进程。**不能用 `python -c "..."`，也不能用 `python - <<EOF` 的 heredoc 写法**——子进程会找不到主模块而报错。必须是真实存在的 `.py` 文件。 |
...
> ⚠️ 易踩坑：最容易踩的两个坑——① 手快打成 `VLLM_METAL_USE_PAGED_ATTENTION=1`
> （方向反了，`0` 才是"退回兼容实现"）；② 图省事用 `python -c` 跑，看到一堆
> 子进程报错。两个坑都在上面表格里，遇到报错先回来对一遍。

> 📌 对标 vLLM：你刚才用的 `LLM` 类，就是真实 vLLM 对用户暴露的门面，
> 源码在 `$VLLM_SRC` 仓库的 `vllm/entrypoints/` 一带。
...
`VLLM_METAL_USE_PAGED_ATTENTION` 写成了 `1`、或用了 `python -c`。
