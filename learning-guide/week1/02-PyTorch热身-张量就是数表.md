...
> 3. 亲手实现 `minivllm/tensor_ops.py` 里的 10 个函数，点亮本周第一批绿灯；
> 4. 为第 3 章埋下关键一发：`last_token_logits`——从模型输出里取出最后位置对所有候选 token 的评分。
...
里面有 11 个函数，其中 `best_device` 已经给好了，剩下 10 个都是 `TODO`——今天全灭。
...
## 2.7 last_token_logits：从很多排预测中取出最后一排

最后一个函数，也是下一章生成文本时最关键的一次索引操作。
先不要急着记答案，我们先弄明白：模型到底输出了什么。

### 模型不是只预测一次，而是每个位置都预测一次
...
输入长度是 3。模型会为三个位置分别算出一排分数：

```text
位置 0：看到“我”之后，下一个 token 可能是什么？
位置 1：看到“我 爱”之后，下一个 token 可能是什么？
位置 2：看到“我 爱 你”之后，下一个 token 可能是什么？
```

如果词表中有 `V` 个候选 token，那么每个位置都有 `V` 个分数。
模型输出的 `logits` 形状是：

```text
(batch, seq_len, vocab_size)
   │       │          └── 词表大小：每个候选 token 一个分数
   │       └───────────── 输入长度：每个位置一排预测
   └───────────────────── 批次大小：这次输入了几句话
```

本周一次只输入一句话，所以形状可以写成 `(1, L, V)`：
...
- 位置 0 在预测“我”后面是什么，但答案已经在输入里了，就是“爱”；
- 位置 1 在预测“我 爱”后面是什么，答案也已经在输入里了，就是“你”；
- 位置 2 在预测完整的“我 爱 你”后面是什么，这才是尚未出现的新 token。

所以：

> **生成下一个 token 时，只需要最后一个位置的那一排分数。**

### 用具体数字看三维 logits

为了方便观察，假设一次输入 1 句话，输入有 3 个位置，词表只有 4 个候选 token：

```python
logits = torch.tensor([
    [
        [0.1, 0.2, 0.3, 0.4],   # 第 0 句话，位置 0
        [1.1, 1.2, 1.3, 1.4],   # 第 0 句话，位置 1
        [2.1, 2.2, 2.3, 2.4],   # 第 0 句话，位置 2
    ]
])
...
把最外层的“第几句话”暂时拿掉，它就像下面这张表：

```text
                    词表里的 4 个候选 token
                 token 0  token 1  token 2  token 3
位置 0             0.1      0.2      0.3      0.4
位置 1             1.1      1.2      1.3      1.4
位置 2（最后）     2.1      2.2      2.3      2.4
```
...
```python
last_position = one_sentence[-1]
last_position.shape
# torch.Size([4])
```

`-1` 是 Python 的负数下标，表示最后一个位置。

第三步，`:` 表示这一维全部都要：

```python
last_position[:]
# tensor([2.1000, 2.2000, 2.3000, 2.4000])
```
...
`last_token_logits` 返回的是所有候选 token 的**分数**：

```python
tensor([2.1, 2.2, 2.3, 2.4])
```

这些分数就叫作 **logits**，它们还不是最终选中的 token。
下一章会用 `argmax` 选出分数最大的下标：

```python
last_logits = last_token_logits(logits)
...
结果是 `3`，因为下标 3 对应的分数 `2.4` 最大。这里的 `3` 才是选中的 token 编号。
...
> **`last_token_logits` 从第 0 句话中，取出最后一个位置对全部候选 token 的评分。**

> 💡 当前函数假设 `batch=1`。如果以后要同时处理多句话，通常会写成
> `logits[:, -1, :]`，返回形状 `(batch, vocab_size)`。本周先只处理一句话。

> 📌 对标 vLLM：真实 vLLM 同样只保留每个请求最新位置的 logits，再把它们送去采样。
> Week 6 我们手写 `Sampler` 时，输入就是这一份 `(V,)` 的分数表。

---

## 2.8 点亮绿灯

10 个函数填完，跑本周测试。先看“张量热身”相关的那批：
...
1. 填完 `minivllm/tensor_ops.py` 里的 10 个 `TODO`（`best_device` 已给出，不用动）。
