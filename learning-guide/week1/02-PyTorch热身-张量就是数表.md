...
`row_sums`（每行求和）和 `column_means`（每列求平均）引出本章最绕的一个概念：**dim（轴）**。
先看代码和结果。例子的表是 **2 行 3 列**——行数和列数不相等，等下从"结果有几个数"
就能一眼判断方向：
...
t.sum(dim=1)    # tensor([ 6., 15.])：每行加起来，2 个数
t.sum(dim=0)    # tensor([ 5.,  7.,  9.])：每列加起来，3 个数
t.mean(dim=1)   # tensor([2., 5.])：每行平均
t.mean(dim=0)   # tensor([2.5, 3.5, 4.5])：每列平均
```

为什么 `sum(dim=1)` 是"每行加"，而不是"每列加"？"dim=0 按列、dim=1 按行"
这种绕口令背了就忘。下面给你一个**永远不错的读法**，三步走。

### 第一步：给每个数贴两个下标

二维张量的每个数，由**两个下标**一起定位：`t[i, j]`。

![给每个数贴两个下标：i 是行号（第 0 维），j 是列号（第 1 维）](../../figures/out/w1_tensor_subscripts.png)

- `i` = 行号（竖着数，第几行）——**这就是第 0 维**；
- `j` = 列号（横着数，第几列）——**这就是第 1 维**。

所以"维度"一点也不抽象：**shape 的第几个数，就是第几个下标**。
`t.shape == (2, 3)` 的意思就是：`i` 能取 2 个值，`j` 能取 3 个值。

### 第二步：dim=几，就消灭第几个下标

"`sum(dim=1)` 消灭了第 1 维"，准确含义是：

> **把下标 `j` 从 0 扫到尾、全部加起来，然后把它从下标里划掉。**

`j` 被划掉了，幸存的只有行号 `i`——每个固定的 `i` 就剩一个数：

```python
result[i] = t[i,0] + t[i,1] + t[i,2]
# i=0：1 + 2 + 3 = 6
# i=1：4 + 5 + 6 = 15
```

结果按幸存的行号 `i` 排列，长度 = 行数 = 2。这就是"消灭列 = 每行加起来"：
**行还活着，每行为了从 3 个格子塌缩成 1 个格子，只能把自己横着加一遍。**

![sum 的两个消灭方向：dim=1 横着压扁每行，dim=0 竖着压扁每列](../../figures/out/w1_tensor_dim_sum.png)

反过来看 `sum(dim=0)`：被扫一遍加起来、然后划掉的是行号 `i`，幸存的是列号 `j`：
`result[j] = t[0,j] + t[1,j]` → 每列一个数，长度 = 列数 = 3。

### 第三步：用形状自查，方向就永远不错

对 sum、mean 这类"把几个数并成一个"的聚合操作，一条公式管所有情况：

> **结果形状 = 原形状把第 dim 维划掉。**

| 操作 | 原形状 | 划掉谁 | 结果形状 | 含义 |
|---|---|---|---|---|
| `t.sum(dim=1)` | (2, 3) | 第 1 维的 3 | (2,) | 每行剩一个数 |
| `t.sum(dim=0)` | (2, 3) | 第 0 维的 2 | (3,) | 每列剩一个数 |

> ⚠️ 易踩坑：检验方向别靠背，靠**形状**。写完 `dim` 先口算结果形状
> （原形状划掉第 dim 维），跑一下打印 `.shape` 对答案——对上了，方向就一定没反。
> 另一个小坑：2×2 的方表"每行加"和"每列加"的结果都是 2 个数，从形状上看不出对错——
> 自己练手时先用行数 ≠ 列数的表（比如 2 行 3 列），方向一验便知。

这套"下标读法"之后到处用得上，先混个脸熟（`dim=几`，操作就沿第几个下标进行）：

- Week 2 注意力：`softmax(scores, dim=-1)`——沿最后一维（键那一维）归一化，每行权重和为 1；
- Week 3 拼接：`torch.cat([ids, new_token], dim=1)`——沿第 1 维（列方向）把新 token 接到序列尾巴；
- Week 6 RMSNorm：`x.pow(2).mean(dim=-1)`——沿最后一维求平均，每个位置得到一个均值。

对应到战场上的两个函数（现在你知道 `dim` 为什么这么填了）：

```python
def row_sums(t):        # 每行求和：划掉列号 j
    return t.sum(dim=1)

def column_means(t):    # 每列求平均：划掉行号 i
    return t.mean(dim=0)
```
