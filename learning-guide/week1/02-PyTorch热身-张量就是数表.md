...
`row_sums`（每行求和）和 `column_means`（每列求平均）引出一个重要概念：**dim（轴）**。

```python
t = torch.tensor([[1., 2.],
                  [3., 4.]])

t.sum(dim=1)    # tensor([3., 7.])：每行一个和
t.mean(dim=0)   # tensor([2., 3.])：每列一个平均
```

`dim=几` 的记忆口诀：**dim=几，就压扁第几维。**

- 表格的第 0 维是"行"（竖着数有几行），第 1 维是"列"（横着数有几列）；
- `t.sum(dim=1)`：压扁"列"这一维 → 每行的所有列加在一起 → 结果长度 = 行数 = 2；
- `t.mean(dim=0)`：压扁"行"这一维 → 每列的所有行取平均 → 结果长度 = 列数 = 2。

> ⚠️ 易踩坑：`dim` 的方向很容易记反。别背"dim=0 是按行求和"这种绕口令，
> 只记"**dim=几就消灭几**"：`sum(dim=1)` 消灭了第 1 维（列），所以每行剩一个数。
> 写完不确定就打印 `.shape` 验证——看形状是张量编程的第一自卫术。
