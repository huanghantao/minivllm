...
> 本章你将：
> 1. 分清 `generate_naive` 的“只给前缀”和训练的“整句并行前向”，
>    解开“未来 token 还没生成，为什么需要掩码”的反直觉问题；
...
先把答案说在最前面：**你如果只盯着 Week 1 的 `generate_naive`，直觉完全正确。**

第一轮生成时：

```python
ids = torch.tensor([[10, 20]])  # 假设 10="我"，20="爱"
logits = model(ids)
next_token = torch.argmax(logits[0, -1, :])
```

输入中根本没有“吃”，最后位置当然无从偷看。因果掩码里的“抄答案”，描述的不是
这次生成调用，而是另一种调用——**训练时把整条正确文本一次送进模型**。

### 第一步：先把两个 `model(...)` 调用摆在一起

我们都看位置 1 的 logits，但输入长度不同：

```python
# A：generate_naive 当前这一轮真正使用的
prefix_ids = torch.tensor([[10, 20]])          # [我, 爱]
A = model(prefix_ids)[0, 1, :]

# B：训练时整句并行前向中的位置 1
full_ids = torch.tensor([[10, 20, 30, 40]])    # [我, 爱, 吃, 苹果]
B = model(full_ids)[0, 1, :]
```

- A 的输入只有 `[我, 爱]`，不存在“吃”；
- B 的输入已经包含 `[我, 爱, 吃, 苹果]`，“吃”就在位置 2；
- 因果掩码要保证的不是“从张量里删掉吃”，而是让 B 的位置 1 **虽然和吃同处一个张量，
  却仍然只能使用 `[我, 爱]` 的信息**。

![真实生成调用与并行训练调用的对照：同取位置 1 logits，但训练输入中答案“吃”已经位于位置 2；因果掩码保证两者等价](../../figures/out/w2_training_vs_generation.png)

这张图最底下的 `A = B`，就是整章真正要守住的合同：
...
这里省略了真实整数 token id；若使用返回对象的 HuggingFace 模型，再在 `model(...)` 后加
`.logits`。右边多放了“吃、苹果”，位置 1 的结果也不许改变。

### 第二步：训练为什么会把整句放进去？

训练不调用 `generate_naive`。`generate_naive` 上还有 `@torch.no_grad()`，本来就是推理代码。
标准的“预测下一个 token”训练，会用一次前向同时做多道题：

```python
import torch
import torch.nn.functional as F

ids = torch.tensor([[10, 20, 30, 40]])       # [我, 爱, 吃, 苹果]
logits = model(ids)                         # shape (1, 4, vocab)

predictions = logits[:, :-1, :]            # 位置 0、1、2 的预测
targets = ids[:, 1:]                        # token id [20, 30, 40]
...
| 模型使用哪一排 logits | 这排在训练中要回答什么 |
|---|---|
| `logits[0, 0, :]` | 根据 `[我]` 预测“爱” |
| `logits[0, 1, :]` | 根据 `[我, 爱]` 预测“吃” |
| `logits[0, 2, :]` | 根据 `[我, 爱, 吃]` 预测“苹果” |

这样一次矩阵计算，就等价于同时练习三个不同长度的前缀，比真的分别调用三次模型快得多。
但并行带来了一个条件：虽然整句都在输入里，**每一排 logits 只能使用属于自己的前缀。**

### 第三步：“看见吃”怎样一路变成“预测吃”？

上一章已经补上了这条桥：Attention 不生成词，它先为位置 1 算出隐藏向量 `h₁`，
`lm_head(h₁)` 才得到位置 1 的整排词表 logits。

没有掩码时，位置 1 的 Attention 输出可以是：

$$
h_1 = a_{1,0}V_{\text{我}} + a_{1,1}V_{\text{爱}}
    + a_{1,2}V_{\text{吃}} + a_{1,3}V_{\text{苹果}}
$$

训练 loss 正在奖励位置 1 把“吃”的 logit 调高。模型很快就能发现一条捷径：
...
它没有直接读取 `targets` 张量，但读取了输入位置 2；那个 token 恰好与位置 1 的训练答案
是同一个“吃”。这就是“抄答案”的精确含义。

下面这张图把 B 这次训练调用单独放大。左半边是没有掩码时的信息通路，右半边是加上
因果掩码后的同一批输入：
...
```text
attention[1, 2] = 0   # 不许读取“吃”
attention[1, 3] = 0   # 不许读取“苹果”
```
...
现在训练调用 B 的位置 1，和真实生成调用 A 一样，都只能根据 `[我, 爱]` 预测“吃”。
这时训练 loss 低，才代表模型真的学会了前缀预测，而不是学会“向右看一格”。

如果浏览器支持 JavaScript，可以在下面按 1→4 重走整条链；最后一步可切换“无掩码 / 有掩码”。
静态图和正文已经包含全部知识，交互页只是用来反复验证自己的理解。

<iframe
  src="./interactive/causal-mask-lab.html"
  title="从 Attention 隐藏向量到 logits，再对比训练与生成的因果掩码交互实验"
  loading="lazy"
  sandbox="allow-scripts"
  style="width: 100%; height: 720px; border: 1px solid #d7dde5; border-radius: 8px; background: transparent;"
></iframe>

[在单独页面中打开交互实验](./interactive/causal-mask-lab.html)

### 最后再放回 prefill 和 decode

- **prompt prefill**：用户的整段提示词也会一次送进模型。每个提示词位置都要建立与训练时
  相同的因果表示，之后才能安全写进 KV cache；
- **单 token decode**：若本次只有 1 个新 query，K/V 里也只有历史和它自己，那么确实没有
  未来位置可挡。这唯一一行权限全是 True，优化 kernel 可以不真的创建 mask；
- **模型的因果合同**仍然不变：第 `i` 个 query，只允许读取位置 `j ≤ i` 的 key，包括自己。

这里的“因果”不用想得很玄：过去可以影响未来，未来不能反过来影响过去。掩码隔离的是
**信息流**，不是把未来 token 从完整训练张量里删除。
